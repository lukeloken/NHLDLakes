...
predvars<-c(names(k_full)[c(2,6, 10, 12,15,16,17:22)], 'lakeconn')

df_sd<-k_full[sd_columns_pix]
# df_sd<-k_full[sd_columns]
# df_mean <-k[mean_columns]
df_median <-k_full[med_columns_pix]
df_morph<-k_full[predvars]


k<-na.omit(k_full[c('Lake', predvars, med_columns_pix, sd_columns_pix)])
...
f1<-k[,med_columns_pix[1]]
f2<-k[,med_columns_pix[2]]
f3<-k[,med_columns_pix[3]]
f4<-k[,med_columns_pix[4]]
f5<-k[,med_columns_pix[5]]
f6<-k[,med_columns_pix[6]]
# f7<-k[,mean_columns[7]]
# f8<-k[,mean_columns[8]]
f9<-k[,med_columns_pix[9]]
f10<-k[,med_columns_pix[10]]
...
xvars<-paste(paste('m', 1:12, sep='', collapse='+'), paste('f', fvars, sep='', collapse="+"), sep="+")
...
for (column in 1:ncol(df_sd)){
  ...
  df<-data.frame( df_median[-which(k$Lake=='LauraLake'),-c(7:8)], df_morph[-which(k$Lake=='LauraLake'),-c(6,8,9,11)], y=df_sd[-which(k$Lake=='LauraLake'),column])
  ...
  print(randomForest::varImpPlot(forest_fit[[column]], main=name))
  dev.off()
...
png(paste0("Figures/RandomForest/GG_AllVars", name, ".png"), res=200, width=8,height=10, units="in")
...
grid.arrange(RFplots[[1]], RFplots[[2]], RFplots[[3]],RFplots[[4]],RFplots[[5]],RFplots[[6]],RFplots[[7]],RFplots[[8]],RFplots[[9]],RFplots[[10]], ncol=2)
...
for (var_nu in 1:length(sd_columns)){
  ...
  png(paste0("Figures/Scatterplots/sd_by_response/sd_", name, "_points.png"), res=200, width=4.5,height=12, units="in")
  ...
  for (xvar in 1:length(predvars)){
    ...
    png(paste0("Figures/Scatterplots/sd_by_response/sd_", name, "_pixels.png"), res=200, width=4.5,height=12, units="in")
