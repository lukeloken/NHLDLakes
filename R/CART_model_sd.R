...
l <- readRDS(file='Data/sdWideTableAllNoNA.rds')
...
column<-1
for (column in 1:length(sd_columns)){
  yname<-sd_columns[column]
  ...
  mtext(sd_labels[column], 1,3)
  
  dev.off()
  
  #create equation for cart model
  equation2 = paste0(yname, " ~ DOC + TotalPUF + Cl + lake_area_ha + lakeconnection + ShorelineIndex + lakeconn")
  
  png(paste0("Figures/Carts/morphonly", yname, ".png"), height=5, width=7, units="in", res=300)
  
  lake_cart = rpart(equation2, method = "anova", data = l_small, control = rpart.control(cp=0.01))
  prp(lake_cart, faclen=0, cex = 0.8, extra = 1, type=3)
  mtext(sd_labels[column], 1,3)
